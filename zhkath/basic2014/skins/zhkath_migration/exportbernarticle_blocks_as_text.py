## Script (Python) "exportbernarticle_blocks_as_text"
...
#    try:
        ...
        if oldobj['blocks']:
            for block in oldobj['blocks']:
                blockcookedtext = '<h3>'+block['blocktitle']+'</h3>'
                #if block['blocksubtitle']:
                #    blockcookedtext += '<p>'+block['blocksubtitle']+'</p>'
                if block['blockdescription']:
                    blockcookedtext += '<p>'+block['blockdescription']+'</p>'
                if block['blocktext'] not in ['','<p>&nbsp;</p>']:
                    blockcookedtext += block['blocktext']
                newcookedtext += blockcookedtext
                
        # cook the subjects for the new index page
        newcookedsubjects = oldobj['subjects']
        if oldobj['blocks']:
            for block in oldobj['blocks']:
                blocksubjects = block['blocksubjects']
                if blocksubjects:
                    newcookedsubjects = newcookedsubjects + blocksubjects
                
        # if subcontents, create a folder:
        if oldobj['files'] or oldobj['links'] or oldobj['othercontentstocopy']:
            ...
            # TODO: set news item as indexpage of newfolder OR set specific view template
